...
[CustomEditor(typeof(CompositeBehavior2D))]
public class CompositeBehaviorEditor : Editor
{
    public override void OnInspectorGUI()
    {
        var compositeBehavior = (CompositeBehavior2D)target;
        ...
        else
        {
            ...
            {
                FlockBehavior2D behavior = compositeBehavior.behaviors[i];
                float weight = compositeBehavior.weights[i];

                _ = EditorGUILayout.BeginHorizontal();
                EditorGUILayout.LabelField(i.ToString(), GUILayout.Width(20));
                compositeBehavior.behaviors[i] = (FlockBehavior2D)EditorGUILayout.ObjectField(behavior, typeof(FlockBehavior2D), false);
    ...
    void AddBehavior(CompositeBehavior2D cb)
    {
        cb.behaviors.Add(null);
        cb.weights.Add(1f);
    }

    void RemoveBehavior(CompositeBehavior2D cb)
    {
